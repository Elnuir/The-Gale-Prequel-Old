...
public class Nibbler : MonoBehaviour
{
    ...
    [SerializeField] private float distanceDistantAttack;
    ...
    [SerializeField] private float touchDamage, distantDamageCooldown, knockbackDuration;

   // public float lastTouchDamageTime, touchDamageCooldown;
    ...
    private EnemyAIPathAndMoveNibbler pathAndMoverNibbler;
    private ChaserNibbler chaserNibbler;
    ...
    [SerializeField]private float firstAttackTimeBase;
    private float firstAttackTimeLeft;
    ...
    void Start()
    {
        firstAttackTimeLeft = firstAttackTimeBase;
        chaserNibbler = GetComponentInChildren<ChaserNibbler>();
        chargineTime = startChargingTime;
        rigidbody2D = GetComponent<Rigidbody2D>();
        currentHealth = maxHealth;
        pathAndMoverNibbler = GetComponent<EnemyAIPathAndMoveNibbler>();
    ...
    void Update()
    {
        ...
        if (facingRight == false && rigidbody2D.velocity.x > 0 && !isHit && !isDead &&
            pathAndMoverNibbler.target.transform.position.x - gameObject.transform.position.x > 0)
        {
            Flip();
        }
        else if (facingRight && rigidbody2D.velocity.x < 0 && !isHit && !isDead &&
                 pathAndMoverNibbler.target.transform.position.x - gameObject.transform.position.x < 0) //Flips
        {
            Flip();
        }
    }
    ...
    void CheckTouchDamage()
    {
        ...
        if (hit != null)
        {
            if (!isDead && !isHit && pathAndMoverNibbler.groundCheck != null)
            {
                firstAttackTimeLeft -= Time.deltaTime;
                if (firstAttackTimeLeft <= 0)
                {
                    isAttacking = true;
                }
            }
    ...
    void CheckDistantDamage()
    {
        ...
        {
            RaycastHit2D distanthit = Physics2D.Raycast(distantDamageCheck.transform.position, -transform.right,
                distanceDistantAttack, whatIsPlayer);

            if (distanthit && pathAndMoverNibbler.groundCheck && !isDead && !isHit &&
                chaserNibbler.isChasingPlayer && !isAttacking)
            {
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(
            new Vector2((touchDamageCheckLeftUp.position.x + touchDamageCheckRightDown.position.x) / 2,
                (touchDamageCheckLeftUp.position.y + touchDamageCheckRightDown.position.y) / 2),
            new Vector2((touchDamageCheckRightDown.position.x - touchDamageCheckLeftUp.position.x),
                (touchDamageCheckLeftUp.position.y - touchDamageCheckRightDown.position.y)));
        Gizmos.DrawLine(distantDamageCheck.transform.position,
            transform.position - transform.right * distanceDistantAttack);
    }
